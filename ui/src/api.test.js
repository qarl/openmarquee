...
describe("live API", () => {
    it("startLive attaches an AbortSignal so a wedged backend can't strand the panel", async () => {
        // Regression for the QA finding from Phase 12.2 review: without
        // a timeout, a hung /api/live/start fetch leaves the panel in
    ...
    it("takeoverLive also attaches an AbortSignal", async () => {
        const fetchMock = mockFetch({
            ok: true,
            status: 200,
            json: async () => ({
        ...
        await takeoverLive("v=0\r\noffer\r\n");
        const init = fetchMock.mock.calls[0][1];
        expect(init.signal).toBeInstanceOf(AbortSignal);
    });

    it("startLive surfaces a 409 with active_session_id pulled from the body", async () => {
        ...
        const err = await startLive("v=0\r\noffer\r\n").catch((e) => e);
        expect(err.code).toBe("live_already_active");
