...
def check_main_wedge():
    ...
    if main_dec == 0 and cur_pos == wedge_state["last_pos_ns"]:
        ...
        if wedge_state["consecutive_stalls"] >= WEDGE_STALL_S:
            # Per QA: any watchdog fire in soak = FAIL not success.
            # Log v10 fd-count and the per-slot dec counters so the
            # journal records the exact wedge fingerprint for code
            # to interpret.
            v10_fds = _count_video10_fds()
            other = opposite_slot(main_bin.slot)
            print(f"[wedge] FIRE -- {main_bin.label} dec=0 + pts "
                  f"stuck at {cur_pos / 1e9:.2f}s for "
                  f"{wedge_state['consecutive_stalls']}s "
                  f"(decA={dec_counters['A']} "
                  f"decB={dec_counters['B']} "
                  f"v10_fds={v10_fds} "
                  f"main.slot={main_bin.slot} other={other}) "
                  "-- force-recover (soak: this is a FAIL signal)",
                  file=sys.stderr)
            wedge_state["consecutive_stalls"] = 0
            wedge_state["last_pos_ns"] = -1
...
def _count_video10_fds():
    ...
    4-8 (1-2 active decoders, each with multiple fd entries for
    the m2m queues)."""
    try:
        ...
        for entry in os.listdir(fd_dir):
            try:
                ...
                if target.startswith("/dev/video10"):
                    count += 1
            except OSError:
                pass
        return count
...
def force_recover_main(bin_):
    ...
    slot_state["outgoing"] = bin_
    slot_state["main"] = new_main
    # Force-teardown the wedged bin immediately (no grace -- the
    # bin has no live in-flight buffers to drain since it produced
    # zero frames; the standard 250ms grace + triple-gate exists
    # to protect a live cross-fade's final painted frame).
    bin_.teardown()
