...
def test_each_fixture_has_required_fields(spec):
    for fx in spec["fixtures"]:
        ...
        assert not missing, f"fixture {fx.get('name')!r} missing required keys: {missing}"
...
def test_kind_specific_fields(spec):
    for fx in spec["fixtures"]:
        if fx["kind"] == "single":
            ...
            assert not missing, f"single fixture {fx['name']!r} missing: {missing}"
        elif fx["kind"] == "transition_mid":
            missing = TRANSITION_MID_KEYS - set(fx)
            assert not missing, f"transition fixture {fx['name']!r} missing: {missing}"
            t = fx["transition_t"]
            assert 0.0 <= t <= 1.0, f"{fx['name']}: transition_t {t} not in [0,1]"
            assert fx["transition"] in TRANSITION_NAMES, (
                f"{fx['name']}: transition {fx['transition']!r} not in {sorted(TRANSITION_NAMES)}"
            )
        else:
...
def test_fixture_uuids_reference_existing_item_json(spec):
    ...
    for fx in spec["fixtures"]:
        ...
        for u in uuids:
            ...
            assert item_path.exists(), f"{fx['name']!r} references {u} but {item_path} missing"
...
def test_fixture_golden_references_existing_png(spec):
    ...
    for fx in spec["fixtures"]:
        ...
        assert png.exists(), f"{fx['name']!r} references golden {fx['golden']!r} but {png} missing"
