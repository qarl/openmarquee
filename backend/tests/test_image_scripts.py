...
def test_build_image_help() -> None:
    ...
    r = subprocess.run(
        ["bash", str(_BUILD), "--help"], check=True, capture_output=True, text=True
    )
...
def test_build_image_unknown_arg_rejected() -> None:
    r = subprocess.run(
        ["bash", str(_BUILD), "--no-such-flag"], check=False,
...
def test_build_image_dry_run_clones_pigen() -> None:
    r = subprocess.run(
        ["bash", str(_BUILD), "--dry-run"], check=True,
        capture_output=True, text=True
    )
...
def test_flash_help() -> None:
    r = subprocess.run(
        ["bash", str(_FLASH), "--help"], check=True,
        capture_output=True, text=True
    )
...
def test_flash_requires_two_positional_args() -> None:
    r = subprocess.run(
        ["bash", str(_FLASH)], check=False, capture_output=True, text=True
    )
    assert r.returncode == 2
...
def test_flash_rejects_missing_image(tmp_path: Path) -> None:
    ...
    r = subprocess.run(
        ["bash", str(_FLASH), "--dry-run", "--yes",
         str(tmp_path / "nope.img"), "/dev/null"],
        check=False, capture_output=True, text=True
    )
    assert r.returncode != 0
...
def test_flash_rejects_missing_device(tmp_path: Path) -> None:
    ...
    r = subprocess.run(
        ["bash", str(_FLASH), "--dry-run", "--yes",
         str(img), "/dev/this-disk-cannot-exist-xyz"],
        check=False, capture_output=True, text=True
    )
    assert r.returncode != 0
