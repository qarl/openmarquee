    ...
    try:
        ...
        try:
            ...
            if returncode != 0:
                ...
                raise WebRenderError(
                    f"Chromium exited {returncode} rendering {url}: "
                    f"{detail}"
                )
...
def _terminate_process_group(proc: subprocess.Popen) -> None:
    ...
    try:
        ...
        # ValueError if pipes were already closed (e.g. clean-exit
        # path already drained inside the main try); TimeoutExpired
        # caught by the wait() fallback below.
        pass
