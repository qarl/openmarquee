...
- **`colorlight.rs` — hardware-agnostic transport surface.** Cross-platform, host-testable. Defines the `FrameSink` object-safe trait (one `send_frame` + default `send_frames` that a real transport overrides with `sendmmsg`), `VecSink` (in-memory recorder for loopback tests), `StubPacketSink` (structural stand-in that always returns `TransportUnavailable` so Phase-0 dispatch code can name the concrete "real transport" type before the socket lands), and the `encode_to_sink` glue helper (encoder → `&mut dyn FrameSink`). See PR #A (2026-07-15) for the trait + fake sinks; see PR #B (compositor → encoder glue) for the render-loop wire-up. **The real `AF_PACKET`/`SOCK_RAW` transport is a follow-up:** a Phase-1 `PacketSink : FrameSink` behind `#[cfg(target_os="linux")]`, opening the raw socket + blasting frames on the configured NIC. Thin — swaps in behind the same `dyn FrameSink` seam with no caller change.
...
- **Compositor → encoder seam:** the compositor stops at RGB888 128×96; `colorlight::encode_to_sink(fb, cfg, &mut dyn FrameSink)` (see §4) is the one-line boundary. Tests point it at a `VecSink`; production points it at the Phase-1 `PacketSink`. Same seam.
