...
#
# cma is 256M (see patch_cmdline_txt_cma). FIRST-LIGHT FIX
# 2026-09-21: the 2026-07-09 GAP2 bump to 320M "validated live-sign"
# value bricked a fresh arm64/trixie COLD boot on a 512MB Zero 2 W
# (gpu_mem=128 + cma=320M -> ~64MB normal RAM -> early-mem OOM before
# fbcon). Reverted to 256M -> ~128MB normal RAM = safe. gpu_mem=128
# is unchanged (needed for the ril.video_decode reloc heap).
#
# Behavior:
...
#
# FIRST-LIGHT FIX 2026-09-21: value REVERTED to 256M. The 320M
# "validated live-sign" value (Handover reconcile 2026-07-09 GAP2)
# was only ever validated on a RUNNING sign that had already booted
# with a smaller CMA — it was NEVER cold-boot-tested from a fresh
# pi-gen image. On a 512MB Pi Zero 2 W a fresh arm64/trixie image
# with gpu_mem=128 + cma=320M leaves only ~64MB normal (non-CMA)
# ARM RAM, so the kernel dies in early memory init BEFORE the
# framebuffer console: blank HDMI, zero kernel output, solid ACT LED
# (reproduced on 2 cards 2026-09-19..21; proven config-only by a
# boot-partition diff — our kernel8.img was BYTE-IDENTICAL to the
# stock RPi OS image that boots). MATH on 512MB:
#   gpu_mem=128 -> 384MB ARM; cma=256M -> ~128MB normal RAM = SAFE.
#   (cma=320M -> ~64MB normal = brick; cma=384M -> even less.)
# See code/docs/first-light-rootcause-cma-2026-09-21.md.
#
